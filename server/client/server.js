...
/** Mesmo diretório que o backend (65432) serve: PulseFlow-Web/client — evita UI antiga na porta 3000. */
...
app.use('/client/public', express.static(path.join(clientRoot, 'public')));
app.use('/client/views', express.static(path.join(clientRoot, 'views')));

app.get('/', (req, res) => {
  res.sendFile(path.join(clientRoot, 'views', 'index.html'));
});

app.get('/login', (req, res) => {
  res.sendFile(path.join(clientRoot, 'views', 'login.html'));
});

app.get('/register', (req, res) => {
  res.sendFile(path.join(clientRoot, 'views', 'register.html'));
});

app.get('/home', (req, res) => {
  res.sendFile(path.join(clientRoot, 'views', 'homePage.html'));
});
...
app.get('/client/views/:filename', (req, res) => {
  ...
  res.sendFile(path.join(clientRoot, 'views', filename));
});

app.listen(PORT, () => {
  console.log(`Frontend servidor rodando na porta ${PORT} (estático: ${clientRoot})`);
